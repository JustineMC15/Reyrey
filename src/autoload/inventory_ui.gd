...
@onready var scripture_grid: GridContainer = $RootPanel/Layout/MiddleColumn/ScriptureGrid
@onready var shard_grid: GridContainer = $RootPanel/Layout/MiddleColumn/ShardGrid
@onready var sword_button: InventoryIconButton = $RootPanel/Layout/LeftColumn/SwordButton
@onready var armor_button: InventoryIconButton = $RootPanel/Layout/LeftColumn/ArmorButton
@onready var shard_currency_button: InventoryIconButton = $RootPanel/Layout/LeftColumn/ShardCurrencyButton

@onready var detail_icon: TextureRect = $RootPanel/Layout/RightColumn/VBox/DetailIcon
@onready var detail_name: Label = $RootPanel/Layout/RightColumn/VBox/DetailName
@onready var detail_keybind: Label = $RootPanel/Layout/RightColumn/VBox/DetailKeybind
@onready var detail_description: Label = $RootPanel/Layout/RightColumn/VBox/DetailScroll/DetailDescription
...
func _populate() -> void:
	...
	for ability_id in GameState.abilities.keys():
		if not GameState.has_ability(ability_id):
			continue
		_spawn_icon(scripture_grid, ability_id, scripture_icons.get(ability_id), _show_scripture)
	...
	for shrine_id in shard_ids:
		_spawn_icon(shard_grid, shrine_id, shard_icons.get(shrine_id, shard_icon), _show_shard)

	armor_button.icon_rect.texture = armor_icons.get(GameState.armor_tier)
	_show_sword()

func _spawn_icon(grid: GridContainer, id: String, icon: Texture2D, on_picked: Callable) -> void:
...
func _show_sword() -> void:
	...
	detail_keybind.hide()
	detail_description.text = "Reyrey's blade, carried since before the road began."
...
func _show_scripture(ability_id: String) -> void:
	...
	if data.has("tin_text"):
		lines.append(data["tin_text"])
	if data.has("reynauld_text"):
		lines.append(data["reynauld_text"])
