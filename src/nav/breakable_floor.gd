...
##
## Scene: CollisionShape2D on collision_layer = 1 (solid ground the
## player can stand on). Optional Sprite2D.
...
func slam_break() -> void:
	...
	if sprite:
		...
		tween.tween_property(sprite, "position:y", sprite.position.y + 40.0, 0.3)
